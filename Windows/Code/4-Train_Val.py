...
def train_val(device, classes, num_epochs, batch_size, loss_function, best_weights, weight_decay, path2weights, 
	...
	with open(diagnostic_path, 'w') as file:
		...
		for epoch in range(num_epochs):
			...
			cm_train_heatmap, cm_train = Model_Utils.c_m(np.array(train_all_labels), np.array(train_all_predictions), classes)
			cr_train_heatmap, cr_train = Model_Utils.c_r(np.array(train_all_labels), np.array(train_all_predictions), classes)
			...
			cm_val_heatmap, cm_val = Model_Utils.c_m(np.array(val_all_labels), np.array(val_all_predictions), classes)
			cr_val_heatmap, cr_val = Model_Utils.c_r(np.array(val_all_labels), np.array(val_all_predictions), classes)
			...
			print("Epoch train loss: %.6f, Epoch val loss: %.6f, accuracy: %.2f"%(training_loss, validation_loss, 100 * val_metric))
			
			#scheduler.step()

			Model_Utils.save_work(epoch, save_interval, checkpoints_folder, model, opt, val_metric) #, scheduler)
			writer.writerow([datetime.datetime.now(), epoch+1, batch_size, training_loss, train_metric, validation_loss, val_metric])
