...
def test(device, classes, batch_size, path2weights, prediction_file, 
	...
	test_loader, test_set = Model_Utils.load_data(
		path = Test_Patches_path, shuffle = False, 
	...
	for i, (inputs, labels) in enumerate(test_loader):
		...
		for j in range(start, end):
			...
			sample_name = str(posix(sample_name)).split("/")[-1][:-5]
	...
	cm_test_heatmap, cm_test = Model_Utils.c_m(
		...
		np.array(test_all_predictions), classes)
	cr_test_heatmap, cr_test = Model_Utils.c_r(
		np.array(test_all_labels), 
		np.array(test_all_predictions), classes)
	
	tb_metrics.add_figure(
		"Test Confusion matrix: ", cm_test_heatmap)
	tb_metrics.add_figure(
		"Test Classification report: ", cr_test_heatmap)	
	tb_metrics.close()

	np.savetxt(
		str(diagnostics_directory) + f"/cm_test.csv", 
		cm_test, delimiter = '\t')
	cr_test.to_csv(
		str(diagnostics_directory) + f"/cr_test.csv", 
		sep = '\t')
	...
	with open(prediction_file, "w") as f:
		...
		for x in range(0, test_len_data):
			writer.writerow([names[x], preds[x], 
				f"{100*confidence_stats[x]:.6}"])
