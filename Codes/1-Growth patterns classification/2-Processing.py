...
def augmentor(patches, classes, maximum):
	...
	for subclass in all_paths.keys():
		...
		print(f"\nSubtype: {subclass}", 
			f"\t Num images to be generated: {num_images_desired}")
		print("Generating new images ...")

		while num_images_generated < num_images_desired:
			path = random.choice(paths)
			image_name = str(psoix(path)).split("/")[-1][:-5]
			rep = 0
			...
			if rep == 0:
				...
				transformed_image = PIL.Image.new(
					"RGB", image_to_transform.size, (255,255,255))
