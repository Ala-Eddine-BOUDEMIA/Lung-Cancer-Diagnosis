...
def generate_patches(patches, classes, all_wsi, overlap, csv_files, 
					annotations, Window_size, compression_factor):
	...
	for file, wsi in zip(xml_files, wsi_paths):
		image_name = str(posix(wsi)).split("/")[-1][:-4]
		img = openslide.OpenSlide(str(wsi))
		with open(Path("/".join([str(csv_files),
				image_name + ".csv"])), "w") as w:
			...
			for region in regions:
				...
				i = 1
				ovrlp = overlap[Text]
				for incre_x in range(int(increment_x//ovrlp)):
					for incre_y in range(int(increment_y//ovrlp)): 
						begin_x = int(
							crop_begin_x + (incre_x * Window_size) * ovrlp)
						end_x = min(crop_end_x, begin_x + Window_size) 
						begin_y = int(
							crop_begin_y + (incre_y * Window_size)* ovrlp)
						...
						patch = img.read_region(
							(begin_x, begin_y), 0, (patch_w, patch_h))

						patch_rgb = Image.new(
							"RGB", patch.size, (255,255,255))
						
						patch_rgb.paste(patch) 
						
						patch_rgb = patch_rgb.resize(
							(patch_w//compression_factor, 
								patch_h//compression_factor), 
							Image.ANTIALIAS)
						
						patch_name = ("_".join(
							[image_name, 
							str(incre_x).zfill(4), str(incre_y).zfill(4), 
						...
						writer.writerow(
							...
							crop_begin_x, crop_begin_y, ovrlp])
